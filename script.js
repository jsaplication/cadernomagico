...
const sketch = function(p) {
  ...
  var colorstatus = window.localStorage.getItem('colorstatus');

  if(colorstatus == null){
    currentColor = '#000000'
  }else if(colorstatus == ''){
    currentColor = '#000000'
  }else if(colorstatus == undefined){
    currentColor = "#000000"
  }else{
    document.querySelector("#colorpaint").value = colorstatus;
    currentColor = colorstatus

  }

  console.log('coloratual: ',currentColor)

  const outputDiv = document.querySelector("#colorpaint");
  outputDiv.addEventListener('change', function() {
      var colorx = outputDiv.value;
      window.localStorage.setItem('colorstatus', colorx);
      p5Sketch.updateCurrentColor(colorx);
  });
  ...
  p.setup = function() {
    ...
    btnSave.addEventListener('click', () => {
  // Capturar o conteúdo do canvas usando html2canvas
  html2canvas(document.getElementById('defaultCanvas0')).then(function(canvas) {
    // Converter o canvas para um URL de dados da imagem
    ...
    // console.log(imgData);
    var img = imgData.split("data:image/jpeg;base64,")[1];
    window.location.href = '.download.'+img;
    // downloadLink.download = 'magic-sketchpad.jpg'; // Nome do arquivo de download
    ...
    // Limpar o elemento de link após o download
